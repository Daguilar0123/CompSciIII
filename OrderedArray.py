...
class OrderedArray(object):
    ...
    def __len__(self):                                      # Special def for len() func
        return self.__nItems                                # Retunr number of items
    ...
    def find(self, item):                       # Find index at or just below
        ...
        while lo <= hi:
            ...
            elif self.__a[mid] < item:          # Else, if item is in upper half
                lo = mid+1                      # then raise the lo boundary
            else:
                hi = mid-1                      # Otherwise, lower the hi boundary
